...
$dataToLog = array(
    ...
    "mpesaReceiptNumber".$res['Body']['stkCallback']['mpesaReceiptNumber'],
);
...
$stmt = $conn->query("SELECT * FROM stkpush ORDER BY ID DESC LIMIT 1");
$stmt->execute();
...
foreach($rows as $row){
    ...
    if($res['Body']['stkCallback']['ResultCode'] == '1032'){
        $sql = $conn->query("UPDATE `stkpush` SET `Status` = 'CANCELLED' WHERE `stkpush`.`ID` = $ID");
        $rs = $sql->execute();
     }else{
        $sql = $conn->query("UPDATE `stkpush` SET `Status` = 'SUCCESS' WHERE `stkpush`.`ID` = $ID");
        $rs = $sql->execute();
     }
